...
const INTERNAL_OPS_SERVICE_URL = SERVICE_URLS.INTERNAL_OPS_SERVICE_URL
...
export class InternalOpsServiceClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        apiLogger.externalCall('internal-ops-service', endpoint, durationMs, false, { statusCode: response.status })
        throw new Error(`Internal Ops Service API error: ${response.statusText}`)
      }

      apiLogger.externalCall('internal-ops-service', endpoint, durationMs, true, { statusCode: response.status })
      return response.json()
    } catch (error) {
      clearTimeout(timeoutId)
      const durationMs = timer()
      apiLogger.externalCall('internal-ops-service', endpoint, durationMs, false)
      throw error
    }
  }
