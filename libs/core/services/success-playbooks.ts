...
import { TIME_DURATIONS, DEFAULT_LIMITS, getDefaultAnalyticsPeriodStart } from '@/lib/config/app-config'
...
export class SuccessPlaybooksService {
  ...
  private static async executeEmailStep(step: PlaybookStep, execution: any): Promise<void> {
    ...
    try {
      ...
      const emailResult = await EnhancedEmailService.sendEmail({
        ...
        text: textBody,
        tags: [
          { name: 'campaign_type', value: 'playbook' },
          { name: 'playbook_id', value: execution.playbook_id },
  ...
  private static async executeSMSStep(step: PlaybookStep, execution: any): Promise<void> {
    ...
    if (twilioAccountSid && twilioAuthToken && twilioFrom) {
      ...
      try {
        ...
        console.error('SMS playbook step error:', err.message)
      }
    } else {
      await supabase.from('cs_sms_logs').insert({
        tenant_id: execution.tenant_id,
  ...
  private static async executeConditionStep(step: PlaybookStep, execution: any): Promise<void> {
    ...
    if (condition.type === 'health_score_above') {
      const { data: health } = await supabase
        .from('cs_customer_health_scores')
        .select('overall_score')
        .eq('tenant_id', execution.tenant_id)
        .eq('customer_email', execution.customer_email)
        .order('calculated_at', { ascending: false })
        .limit(1)
        .maybeSingle()
      result = (health?.overall_score || 0) >= (condition.threshold || 50)
    } else if (condition.type === 'csat_above') {
      const { data } = await supabase
        ...
        .limit(1)
        .maybeSingle()
      result = (data?.score || 0) >= (condition.threshold || 4)
    } else if (condition.type === 'usage_growing') {
      result = true
    } else if (condition.type === 'renewal_probability_above') {
      const { data: renewal } = await supabase
        ...
        .limit(1)
        .maybeSingle()
      result = (renewal?.renewal_probability || 0) >= (condition.threshold || 50)
    } else if (condition.type === 'days_since_last_activity_above') {
      const { data: lastEvent } = await supabase
        .from('cs_usage_events')
        .select('event_timestamp')
        .eq('tenant_id', execution.tenant_id)
        .eq('user_id', execution.customer_email)
        .order('event_timestamp', { ascending: false })
        .limit(1)
        .maybeSingle()
      if (lastEvent) {
        const daysSince = Math.floor((Date.now() - new Date(lastEvent.event_timestamp).getTime()) / (1000 * 60 * 60 * 24))
        result = daysSince >= (condition.threshold || 30)
      } else {
        result = true
      }
    }

    const nextOnTrue = step.step_actions?.on_true
    const nextOnFalse = step.step_actions?.on_false

    if (result && nextOnTrue) {
      await supabase.from('cs_playbook_step_executions')
        .update({ step_result: { branch: 'true', next_step: nextOnTrue } })
        .eq('execution_id', execution.execution_id)
        .eq('step_id', step.step_id)
    } else if (!result && nextOnFalse) {
      await supabase.from('cs_playbook_step_executions')
        .update({ step_result: { branch: 'false', next_step: nextOnFalse } })
        .eq('execution_id', execution.execution_id)
        .eq('step_id', step.step_id)
    }
  }
