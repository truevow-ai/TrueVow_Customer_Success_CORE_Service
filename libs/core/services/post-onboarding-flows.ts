...
import { TIME_DURATIONS, getDateInPast } from '@/lib/config/app-config'
...
export class PostOnboardingFlowsService {
          ...
          if (customer.assigned_csm_id) {
            try {
              ...
              await internalOpsClient.createTask({
                ...
                assigned_to: customer.assigned_csm_id,
  ...
  static async processUsageAlerts(
    ...
    try {
      ...
      const usagePattern = await UsageAnalyticsService.calculateUsagePattern(
        tenantId,
        userId,
        getDateInPast(TIME_DURATIONS.DEFAULT_ANALYTICS_PERIOD_DAYS),
        new Date()
      )
        ...
        if (customer.assigned_csm_id) {
          try {
            ...
            await internalOpsClient.createTask({
              ...
              description: `Customer has low usage (${usagePattern.features_used_count} features used, expected ${expectedUsage}). Last activity: ${usagePattern.days_since_last_activity} days ago.`,
              assigned_to: customer.assigned_csm_id,
              priority: usagePattern.days_since_last_activity > 14 ? 'high' : 'medium',
              related_customer_id: customerId,
              service: 'cs-support',
              metadata: {
                alert_type: 'low_usage',
                current_usage: usagePattern.features_used_count,
                expected_usage: expectedUsage,
                days_since_last_use: usagePattern.days_since_last_activity,
              },
            })
          } catch (error) {
            console.error('Failed to create task in Internal Ops:', error)
