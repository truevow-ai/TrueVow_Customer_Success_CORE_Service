...
export class UsageAnalyticsService {
  ...
  static async calculateFeatureAdoption(
    ...
    for (const featureName of uniqueFeatures) {
      ...
      const totalTenantUsers = new Set(allTenantUsers?.map(u => u.user_id) || []).size || 1
  ...
  static async calculateUsagePattern(
    ...
    // Calculate average session duration from consecutive event timestamps
    const sessionDurationAvg = await this.calculateSessionDuration(tenantId, userId)
  ...
  /**
   * Calculate average session duration in minutes from event timestamps
   */
  private static async calculateSessionDuration(
    tenantId: string,
    userId: string
  ): Promise<number> {
    const supabase = await createServerSupabase()
    ...
    if (!events || events.length < 2) return 15

    const INACTIVITY_GAP_MINUTES = 30
    const sessions: number[] = []
    let sessionStart = new Date(events[0].event_timestamp).getTime()
    let lastTs = sessionStart

    for (let i = 1; i < events.length; i++) {
      const currentTs = new Date(events[i].event_timestamp).getTime()
      const gapMs = currentTs - lastTs

      if (gapMs > INACTIVITY_GAP_MINUTES * 60 * 1000) {
        sessions.push((lastTs - sessionStart) / (1000 * 60))
    ...
    const avg = sessions.reduce((sum, d) => sum + d, 0) / sessions.length
  ...
  static async generateAnalyticsSummary(
    ...
    // Calculate new users from first-seen events within each window
    const { data: allTimeUsers } = await supabase
      .from('cs_usage_events')
    ...
    const newUsers7d = Array.from(userFirstSeen.entries())
      .filter(([, ts]) => ts >= sevenDaysAgo).length

    const newUsers30d = Array.from(userFirstSeen.entries())
      .filter(([, ts]) => ts >= thirtyDaysAgo).length

    const totalAllTimeUsers = userFirstSeen.size
    const userGrowthRate = totalAllTimeUsers > 0
      ? Math.round((newUsers30d / totalAllTimeUsers) * 10000) / 100
      : 0
