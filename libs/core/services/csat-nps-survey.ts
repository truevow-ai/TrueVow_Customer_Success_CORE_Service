...
export class CSATNPSSurveyService {
  ...
  private static async getApplicableRules(
    ...
    for (const rule of allRules) {
      ...
      if (rule.exclude_if_reopened) {
        ...
        const wasReopened = activityLog?.some(
          (entry: any) => entry.old_status === 'closed' && entry.new_status === 'open'
        )
        if (wasReopened) continue
      }
  ...
  private static async queueSurvey(
    ...
    // Survey is processed by the cron endpoint (`POST /api/v1/surveys/send-scheduled`)
    // which calls sendScheduledSurveys() on a regular interval.
    // No background job infrastructure needed — cron handles dispatching.
  }
  ...
  private static async sendSMSSurvey(
    ...
    const supabase = await createServerSupabase()
    const body = `${template.message_text || 'How was your experience?'}\n\n${surveyLink}`
    ...
    try {
      ...
      if (twilioAccountSid && twilioAuthToken && twilioFrom) {
        ...
        if (res.ok) {
          ...
          console.error('Twilio SMS survey send error:', errorText)
        }
      } else {
        await supabase.from('cs_sms_logs').insert({
          tenant_id: survey.tenant_id,
      ...
      console.error('Error sending SMS survey:', error.message)
    }
  }
  ...
  static async getSurveyStats(
    ...
    const { count: csatSentCount } = await supabase
      .from('cs_survey_csat')
    ...
    const { count: npsSentCount } = await supabase
      .from('cs_survey_nps')
      .select('*', { count: 'exact', head: true })
      .eq('tenant_id', tenantId)
      .eq('auto_sent', true)
    ...
    const totalSent = (csatSentCount || 0) + (npsSentCount || 0)
    const totalResponded = csatScores.length + totalNPS
    ...
    return {
      ...
      response_rate: {
        ...
        surveys_responded: totalResponded,
