...
import { TIME_DURATIONS, getReportExpirationDate } from '@/lib/config/app-config'
...
export class ReportGeneratorService {
    ...
    try {
      ...
      const { data: report, error: reportError } = await supabase
        ...
        .insert({
          ...
          expires_at: getReportExpirationDate().toISOString(),
        })
        .select()
        .single()
      ...
      if (report.report_format === 'pdf' && process.env.FEATURE_PDF_GENERATION === 'True') {
        const htmlContent = this.generatePdfHtml(report)
        await supabase
          .from('cs_generated_reports')
          .update({ pdf_html: htmlContent, pdf_generated_at: new Date().toISOString() })
  ...
  private static async getTicketData(
    ...
    // Get volume and resolution metrics from separate calls
    const [volumeMetrics, resolutionMetrics] = await Promise.all([
      AnalyticsService.getTicketVolumeMetrics(tenantId, { from: periodStart, to: periodEnd }),
      AnalyticsService.getResolutionTimeMetrics(tenantId, { from: periodStart, to: periodEnd }),
    ])

    const total = volumeMetrics.total || 0
    const resolved = volumeMetrics.resolved || 0
    const open = volumeMetrics.open || 0
    ...
    return {
      ...
      avg_resolution_time: resolutionMetrics.average || 0,
      by_channel: volumeMetrics.byChannel || {},
      by_status: volumeMetrics.byStatus || {},
      by_priority: volumeMetrics.byPriority || {},
    }
  }
  ...
  private static async getAgentPerformanceData(
    ...
    const agentRankings = await AgentPerformanceService.getAgentRankings('satisfaction')

    return {
      total_agents: teamMetrics.total_agents,
      active_agents: teamMetrics.total_agents, // same as total for now
      total_tickets: teamMetrics.total_tickets,
      total_resolved: Math.round(teamMetrics.total_tickets * (teamMetrics.resolution_rate / 100)),
      avg_resolution_rate: teamMetrics.resolution_rate,
      avg_response_time: teamMetrics.avg_first_response_time,
      avg_resolution_time: 0, // not available in TeamPerformanceSummary
      avg_csat_score: teamMetrics.avg_satisfaction,
      avg_sla_compliance: 0, // not available in TeamPerformanceSummary
      top_performers: agentRankings.slice(0, 3),
      needs_improvement: agentRankings.slice(-2),
    }
  }
  ...
  private static async getUsageAnalyticsData(
    ...
    const summary = await UsageAnalyticsService.getAnalyticsSummary(
      tenantId,
    ...
    return {
      active_users_7d: summary?.activeUsers7d || 0,
      active_users_30d: summary?.activeUsers30d || 0,
  ...
  private static async getCustomerSatisfactionData(
    ...
    const stats = await CSATNPSSurveyService.getSurveyStats(
      tenantId,
      new Date(periodStart),
      new Date(periodEnd)
    )
    ...
    return {
      csat_score: stats?.csatScore || 0,
      csat_count: stats?.csatCount || 0,
      nps_score: stats?.npsScore || 0,
      nps_count: stats?.npsCount || 0,
  ...
  private static generatePdfHtml(report: any): string {
    ...
    const title = report.report_name || 'TrueVow Report'
    const generatedDate = new Date(report.generated_at || Date.now()).toLocaleDateString()
    ...
    const summaryRows = data.summary
      ? Object.entries(data.summary)
          ...
          .join('')
      : '<tr><td colspan="2">No summary data available</td></tr>'
...
<style>
  ...
  .section h2 { font-size: 18px; border-bottom: 2px solid #0066cc; padding-bottom: 4px; }
</style></head><body>
<h1>${title}</h1>
<div class="meta">Generated: ${generatedDate} | Report ID: ${report.report_id}</div>
<div class="section"><h2>Summary</h2><table>${summaryRows}</table></div>
</body></html>`
  }
}
