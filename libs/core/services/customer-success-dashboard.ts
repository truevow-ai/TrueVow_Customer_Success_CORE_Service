 ...
 * 
 * Provides data for Client Success Manager dashboard focused on COMPLETE customer lifecycle management:
 * - Pre-onboarding customers (awaiting onboarding)
 * - Onboarding customers (in progress)
 * - Post-onboarding customers (transferred from SaaS Admin after go-live)
 * - Customer health scores and churn risk
 * - Communication history and escalations
 * - At-risk customer identification
 * - Escalation management for unresolved tickets
 * 
 * NOTE: Onboarding workflows are coordinated with SaaS Admin service.
 * This service manages the complete customer journey from pre-onboarding through post-onboarding success.
 */
...
export interface OnboardingCustomer {
  ...
  status: 'pre_onboarding' | 'onboarding_call' | 'configuration' | 'go_live' | 'post_onboarding' | 'at_risk' | 'completed' | 'active' | 'healthy'
...
export class CustomerSuccessDashboardService {
   ...
   * 
   * Returns all customers for CS dashboard (team members support all customers)
   * CSMs manage customers after they've been onboarded by Client Onboarding Managers
   */
  static async getDashboardData(_userId: string): Promise<CustomerSuccessDashboardData> {
    const supabase = await createServerSupabase()

    // Get all post-onboarding customers (team members support all tenants)
    ...
    if (!postOnboardingCustomers || postOnboardingCustomers.length === 0) {
      return {
        ...
        post_onboarding_customers: [],
        at_risk_customers: [],
        escalations_needing_attention: [],
        communication_stats: {
    ...
    const customers = await Promise.all(
      postOnboardingCustomers.map(async (customer) => {
        ...
        if (healthScore === null) {
          try {
            const healthScoreData = await HealthScoringService.calculateHealthScore(
              customer.tenant_id,
              customer.customer_email
            )
            healthScore = healthScoreData?.health_score || null
          } catch (error) {
    ...
    return {
      summary: {
        ...
        pre_onboarding: 0, // Not applicable for post-onboarding dashboard
        in_onboarding: 0, // Not applicable for post-onboarding dashboard
        post_onboarding: totalCustomers,
        at_risk: atRiskCustomers.length,
        average_health_score: Math.round(averageHealthScore),
        escalations_today: 0, // Would need escalation tracking
        unresolved_tickets: 0, // Would need ticket tracking
      },
      pre_onboarding_customers: [], // Empty for post-onboarding dashboard
      onboarding_customers: [], // Empty for post-onboarding dashboard
      post_onboarding_customers: customers,
      at_risk_customers: atRiskCustomers,
      escalations_needing_attention: [], // Would need escalation tracking
      communication_stats: {
   ...
   */
  static async getCustomerDetails(customerId: string): Promise<OnboardingCustomer | null> {
    const supabase = await createServerSupabase()
