...
export class BillingProxyService {
  ...
  private static platformServiceUrl = process.env.PLATFORM_SERVICE_URL || 'http://localhost:3000'
  ...
  static async executeBillingOperation(
    ...
    if (sanitizedOperation.amount !== undefined) {
      if (sanitizedOperation.amount < 0 || sanitizedOperation.amount > 1000000) {
        throw new Error('Invalid amount: must be between 0 and 1,000,000')
      }
    }

    if (sanitizedOperation.discountPercent !== undefined) {
      if (sanitizedOperation.discountPercent < 0 || sanitizedOperation.discountPercent > 100) {
        throw new Error('Invalid discount: must be between 0 and 100 percent')
      }
    }
