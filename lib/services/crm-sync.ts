...
export class CRMSyncService {
  private static crmApiUrl = process.env.INTAKELY_CRM_API_URL || 'https://api.intakely.com'
  ...
  static async updateCaseInCRM(ticketId: string): Promise<CRMCase> {
    ...
    const sanitizedCaseId = validateInput(syncStatus.crmCaseId, 'uuid')
    ...
    try {
      const response = await fetch(`${this.crmApiUrl}/v1/cases/${sanitizedCaseId}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${this.crmApiKey}`,
          'Content-Type': 'application/json',
