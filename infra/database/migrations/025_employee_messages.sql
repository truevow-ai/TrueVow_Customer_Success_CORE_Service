...
-- CS Core must NOT store or reference lead data.
-- Lead data resides exclusively in SaaS Admin.
-- This table stores CS support messages only.
-- lead_id column has been removed (was a boundary violation).
-- service_type is cs_support only (sales_crm references removed).
...
CREATE TABLE IF NOT EXISTS employee_messages (
  ...
  -- lead_id was REMOVED: lead data resides exclusively in SaaS Admin, not CS Core
  contact_id UUID, -- References contact within CS-Support
  ...
  service_type TEXT CHECK (service_type IN ('cs_support')), -- CS Core only — sales_crm removed (boundary violation)
...
-- lead_id index removed: lead data is not stored in CS Core (boundary violation fixed)
...
-- lead_id column removed: CS Core must NOT own or reference lead data
COMMENT ON COLUMN employee_messages.contact_id IS 'Associated contact ID within CS-Support';
