 ...
 * Usage Spikes API
 * 
 * GET /api/v1/expansion/spikes?tenant_id=...&status=...
 * Get detected usage spikes
 */
...
export const GET = withRateLimit(
  ...
  withTeamMember(async (req: NextRequest, context) => {
    try {
      ...
      const spikes = await ExpansionTriggersService.getUsageSpikes(tenantId, status, limit)

      return successResponse(spikes)
    } catch (error) {
      return errorResponse(
        error instanceof Error ? error.message : 'Failed to get usage spikes',
        500
      )
    }
  })
