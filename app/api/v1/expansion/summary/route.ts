/**
 * Expansion Summary API
 * 
 * GET /api/v1/expansion/summary?tenant_id=...&period_start=...&period_end=...
 * Get expansion summary for dashboard
 */
...
export const GET = withRateLimit(
  ...
  withTeamMember(async (req: NextRequest, context) => {
    try {
      ...
      const summary = await ExpansionTriggersService.getExpansionSummary(tenantId, periodStart, periodEnd)

      return successResponse(summary)
    } catch (error) {
      return errorResponse(
        error instanceof Error ? error.message : 'Failed to get expansion summary',
        500
      )
    }
  })
