 ...
 * Pain Points API
 * 
 * GET /api/v1/analytics/trends/pain-points?tenant_id=...&period_start=...&period_end=...
 * Get identified pain points
 */
...
export const GET = withRateLimit(
  ...
  withTeamMember(async (req: NextRequest, context) => {
    try {
      ...
      const painPoints = await TrendAnalysisService.identifyPainPoints(tenantId, periodStart, periodEnd)

      return successResponse(painPoints)
    } catch (error) {
      return errorResponse(
        error instanceof Error ? error.message : 'Failed to get pain points',
        500
      )
    }
  })
